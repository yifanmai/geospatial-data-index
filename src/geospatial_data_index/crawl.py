...
def set_crawl_status(catalog_destination_directory: Path, status: str) -> str:
...
def add_derived_from_link(stac_object: STACObject):
    if not stac_object.get_single_link(DERIVED_FROM):
        stac_object.add_link(Link(rel=DERIVED_FROM, target=stac_object.get_self_href(), media_type="application/json"))
...
def normalize_and_save_catalog(catalog: Catalog, destination_path: Path, id_strategy: str = USE_ID) -> None:
    ...
    if not catalog.get_single_link(DERIVED_FROM):
        catalog.add_link(Link(rel=DERIVED_FROM, target=catalog.get_self_href(), media_type="application/json"))
...
def normalize_and_save_item(item: Item, collection_destination_directory: Path, id_strategy: str = USE_ID) -> None:
    sleep(0.1)
    ...
    if not item.get_single_link(DERIVED_FROM):
        item.add_link(Link(rel=DERIVED_FROM, target=item.get_self_href(), media_type="application/json"))
